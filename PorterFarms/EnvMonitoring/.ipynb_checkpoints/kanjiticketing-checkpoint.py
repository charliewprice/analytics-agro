...
def slackticket(nodename, location, description, impact, urgency, locationimageurl, SLACK_TOKEN, _SLACK_CHANNEL, ticketid):
        ...
        blockmessage[0]["text"]["text"] = "*{}* \
                        \nat {} \
                        \n*{} {}* \
                        \nticket #{} impact={} urgency={}".format(location, timestamp.strftime("%-I:%M %p %A, %B %e, %Y"), nodename, description, ticketid, impact, urgency)

        blockmessage[2]["text"]["text"] = "*To handle this issue, click the button...*"
        blockmessage[2]["accessory"]["text"]["text"] = "I got it!"
        blockmessage[2]["accessory"]["value"] = "accept"
        blockmessage[2]["accessory"]["action_id"] = "{0}".format(ticketid)
        # end key replacement
...
def openticket(conn, nodeid, locationid, description, impact, urgency, type):
    sqlinsert = "INSERT INTO kanji_ticket (opentimestamp, lastupdatetimestamp, node_id, location_id, description, impact_id, urgency_id, type_id, status_id) \
    VALUES ('{}', '{}', '{}', '{}', '{}', {}, {}, {}, {}) ".format(datetime.now(), datetime.now(), nodeid, locationid, description, impact, urgency, type, _OPEN_STATUS)
...
def updateTicket(conn, idticket):
    updatequery = "UPDATE kanji_ticket SET lastupdatetimestamp = NOW() WHERE idticket={}".format(idticket)
